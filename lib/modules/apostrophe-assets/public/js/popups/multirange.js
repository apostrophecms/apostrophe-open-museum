$(function () {
  ...
  // Events
  $mr.find('[data-multirange-submit]').on('click', function (e) {
    e.preventDefault();
    e.stopPropagation();
  ...
  $mr.find('[data-multirange-input]').change(function() {
    let $this = $mr.find('[data-multirange-input]:not(.ghost)');
  ...
  function resetRange() {
    let $this = $mr.find('[data-multirange-input]:not(.ghost)');
    let data = [$this.attr('min'), $this.attr('max')];
    updateRange(data);
